...
class BoingWindow : public glfw::Window {
public:
  BoingWindow() : glfw::Window( 400, 400, "Boing (classic Amiga demo)", nullptr, nullptr ) {
    width = GetWidth();
    height = GetHeight();
    ...
    reshape(GetGLFWwindow(), width, height);
    init();
  }
  ...
  virtual void MouseButton(int button, int action, int mods) override {
    mouse_button_callback(GetGLFWwindow(), button, action, mods);
  }

  virtual void CursorPos(double xpos, double ypos) override {
    cursor_position_callback(GetGLFWwindow(), xpos, ypos);
  }
