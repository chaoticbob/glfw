...
class ParticlesWindow : public glfw::Window {
public:
  ParticlesWindow(int width, int height, const char* title, GLFWmonitor* monitor, GLFWwindow* share) 
      : glfw::Window(width, height, "Particle Engine", monitor, share) 
  {
    MakeContextCurrent();
    ...
    resize_callback(GetGLFWwindow(), GetWidth(), GetHeight());
  ...
  virtual void FramebufferSize(int w, int h) override {
    resize_callback(GetGLFWwindow(), w, h);
  }

  virtual void Key(int key, int scancode, int action, int mods) override {
    key_callback(GetGLFWwindow(), key, scancode, action, mods);
  }

  void Draw() {
    draw_scene(GetGLFWwindow(), glfwGetTime());
  }
};
...
public:
  ParticlesApp(int argc, char** argv) : glfw::Application() {
    ...
    if (thrd_create(&physics_thread, physics_thread_main, mWindow->GetGLFWwindow()) != thrd_success)
    {
        glfwTerminate();
        exit(EXIT_FAILURE);
    }
...
private:
  ParticlesWindow*  mWindow = nullptr;
  thrd_t  physics_thread = 0;
};
